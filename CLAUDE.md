...
- **Universal ingestion pipeline**: `POST /api/v1/knowledge/ingest` accepts any content type. SourceClassifier routes to deterministic extractors (markdown, config, code, HTML, PDF, structured data). MergeBuffer decides if LLM synthesis adds value. `agency knowledge ingest <file-or-url>` CLI. Watch mode for auto-ingestion directory.
...
- **Provenance-based quarantine**: `agency admin knowledge quarantine --agent <name> [--since timestamp]`. Quarantined nodes excluded from all retrieval. Edge exclusion for edges touching quarantined nodes. Release per-node or per-agent.
- **Knowledge classification**: Four tiers (public/internal/restricted/confidential) defined in `~/.agency/knowledge/classification.yaml`. Classification maps to scope via operator-configurable tier→principal mappings. Auto-scope merge at `add_node()` time. `agency knowledge classification show`.
- **Dynamic routing optimizer**: Background goroutine in gateway. Aggregates LLM call data per (task_type, model). Computes success rate (absence of retry), real USD costs via CalculateCost(). Generates suggestions when cheaper model has ≥90% success, ≥30% savings, ≥20 calls. Operator approves via `agency routing approve <id>` → writes `routing.local.yaml`. `agency routing suggestions/approve/reject/stats`.
...
- `agency routing` manages the dynamic routing optimizer: `agency routing suggestions/approve/reject/stats`. `agency routing approve <id>` writes to `routing.local.yaml`.
- `agency knowledge ingest <file-or-url>` ingests content into the knowledge graph. `agency knowledge classification show` displays classification tiers. `agency admin knowledge quarantine --agent <name> [--since timestamp]` quarantines agent-produced knowledge nodes.
