...
- **Container topology**: Per agent: workspace + enforcer. Shared: egress, comms, knowledge, intake, embeddings (Ollama, conditional on KNOWLEDGE_EMBED_PROVIDER=ollama). Hub-and-spoke network: `agency-gateway` (internal bridge) is the hub — gateway proxy, comms, knowledge, intake all attach here. `agency-egress-int` (internal) connects services to the egress proxy. `agency-egress-ext` (external) connects egress to the internet. `agency-operator` (external) hosts web UI and relay. Per-agent `agency-<name>-internal` networks connect workspace↔enforcer. `CreateMediationNetwork()` and `CreateInternalNetwork()` enforce Internal:true (no external route). Container paths: `/agency/enforcer/`, `/agency/agent/`. No shell sidecar — execution-layer confinement is handled by a custom seccomp profile applied to workspace containers (~100 allowed syscalls).
