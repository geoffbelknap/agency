...
- **Egress**: mitmproxy with credential swap. Only component holding real API keys. Resolves credentials from the gateway's encrypted credential store via Unix socket (`~/.agency/run/gateway.sock`), not from `.service-keys.env` (deleted). `SocketKeyResolver` replaces `FileKeyResolver` as primary. JWT exchange, API key injection, and GitHub App tokens are all driven by `credential-swaps.yaml` which is generated from the credential store.
...
1. **Never construct HostConfig directly** — use `containers.HostConfigDefaults(role)` and overlay specific fields (Binds, NetworkMode, Tmpfs, ExtraHosts)
...
- Container-to-gateway communication uses TCP via `host.docker.internal` for enforcers (budget/signal relay). Gateway also exposes a Unix socket at `~/.agency/run/gateway.sock` for infra containers (egress) to resolve credentials. The socket router has restricted endpoints (no auth needed — access controlled by bind mount). All containers that call the gateway via HTTP need `ExtraHosts: ["host.docker.internal:host-gateway"]`.
