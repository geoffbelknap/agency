...
- **Context compression**: Large tool outputs (>2000 tokens) are summarized by a cheap LLM call before entering the messages array. Compression calls tagged `X-Agency-Cost-Source: context_compression` route to mini tier via hybrid routing. Raw outputs stored in memory and retrievable via `recall_tool_output` built-in tool. Configuration in mission YAML `context:` block or cost_mode defaults.
