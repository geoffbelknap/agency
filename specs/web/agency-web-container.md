...
- **Image**: Multi-stage build. Stage 1: `node:22-alpine` builds the React app. Stage 2: `alpine:3.21` with nginx serves `dist/` over HTTP.
- **Networking**: Host networking mode. No agency-managed Docker networks. Serves HTTP on port 8280. The browser makes API calls to the same origin — nginx proxies them to the gateway.
...
- **Health check**: `wget -q -O /dev/null http://127.0.0.1:8280/health || exit 1`
