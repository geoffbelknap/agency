...
```bash
scripts/dev/dev-api-get.sh --gateway 'graph/memory?type=procedural&agent=jarvis'
scripts/dev/dev-api-get.sh --gateway 'graph/memory/proposals?status=needs_review'
```
