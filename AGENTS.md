...
working inside the main runtime repository. Anthropic tooling (Claude Code)
reads `CLAUDE.md`, which imports this file via `@AGENTS.md`.
...
- user-facing docs (rendered by Mintlify) belong in `docs/`
- experimental user-facing docs belong in `docs/experimental/`
- operator runbooks (incident response, recovery, ops) belong in `docs/runbooks/`
- experimental operator runbooks belong in `docs/runbooks/experimental/`
- architecture specs belong in top-level `specs/` (organized by domain subdirectory; cross-cutting at root)
- release/validation checklists (engineering QA) belong in `tests/checklists/`
- implementation plans belong in `docs/plans/` and are deleted once fully implemented
