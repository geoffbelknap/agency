...
- Docker, Podman, and `containerd` PR smoke workflows use
  `scripts/ci/ci-backend-smoke-needed.sh` to skip expensive runtime setup when a
...
```bash
./scripts/readiness/apple-container-smoke.sh --skip-build
```
...
bash ./scripts/readiness/runtime-contract-smoke.sh --agent <agent-name>
AGENCY_SOURCE_HOME=/tmp/agency-podman-seed.<id> ./scripts/e2e/e2e-live-disposable.sh --skip-build
gh workflow run "Podman Readiness" --ref main -f full_e2e=true
```
...
- **Durable wait-backed events.** The current helper supports verified command lifecycle and home-scoped cleanup in the manual smoke path, and `scripts/readiness/apple-container-wait-helper-smoke.sh` proves Apple `ClientProcess.wait()` can emit an exit event. The normal Apple lifecycle path now starts containers through the wait helper, and Gateway seeds Apple event streams from one bounded helper reconciliation snapshot after restart. Before promotion, either add a durable wait-helper supervisor/IPC shape or accept/document the Apple platform limitation that exit waits cannot be reattached to containers that were already running before Gateway restarted.
- **Topology and recovery evidence.** The normal Apple lifecycle path realizes multi-network intent at create time. Retain smoke evidence that no Docker-shaped post-create network attach warnings occur, Gateway restart preserves runtime contract checks, lifecycle controls still work after restart, and cleanup leaves only Apple Container's built-in resources.
