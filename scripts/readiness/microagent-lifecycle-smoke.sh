...
AGENCY_BIN="${AGENCY_BIN:-/tmp/agency-microagent-lifecycle}"
MICROAGENT_BIN="${AGENCY_MICROAGENT_BIN:-microagent}"
MKE2FS_PATH="${AGENCY_MKE2FS:-}"
ENFORCER_BIN="${AGENCY_MICROAGENT_ENFORCER_BIN:-/tmp/agency-enforcer-host}"
ENFORCER_OCI_REF="${AGENCY_MICROAGENT_ENFORCER_OCI_REF:-}"
AGENT_NAME="${AGENT_NAME:-microagent-lifecycle-$(date +%s)}"
ROOTFS_SIZE_MIB="${AGENCY_MICROAGENT_ROOTFS_SIZE_MIB:-1024}"
...
usage() {
  cat <<'EOF'
Usage: ./scripts/readiness/microagent-lifecycle-smoke.sh [options]

Runs a disposable macOS microagent lifecycle smoke through the public gateway:
  1. verifies microagent and builds or extracts the host-process enforcer
...
Options:
  ...
  --agency-bin PATH       Agency binary output path. Defaults to /tmp/agency-microagent-lifecycle.
  --microagent-bin PATH   microagent CLI path. Defaults to PATH lookup.
  ...
  --enforcer-oci-ref REF  Versioned enforcer OCI artifact reference. When set,
                          extracts the host-platform /usr/local/bin/enforcer and
...
Environment:
  ...
  AGENCY_MKE2FS
  AGENCY_MICROAGENT_ENFORCER_BIN
  AGENCY_MICROAGENT_ENFORCER_OCI_REF
  AGENCY_MICROAGENT_ROOTFS_OCI_REF
  AGENCY_MICROAGENT_ROOTFS_SIZE_MIB
EOF
}
...
cleanup() {
  ...
  if [[ "$KEEP_AGENT" != "1" && -n "$GATEWAY_PORT" && -n "$TOKEN" && -n "$AGENT_NAME" ]]; then
    curl -fsS -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
      ...
      -d '{"type":"immediate","reason":"microagent lifecycle smoke cleanup"}' >/dev/null 2>&1
  ...
  if [[ "$KEEP_HOME" != "1" && -n "$SMOKE_HOME" && "$SMOKE_HOME" == /tmp/agency-microagent-lifecycle.* ]]; then
    rm -rf "$SMOKE_HOME"
  fi
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --agency-bin)
      ...
      ;;
    --microagent-bin)
      [[ $# -ge 2 ]] || fail "--microagent-bin requires a path"
      MICROAGENT_BIN="$2"
      shift 2
      ;;
    --mke2fs)
...
api_stream_expect() {
  ...
  local body="${3:-}"
  local out
  out="$(mktemp /tmp/agency-microagent-stream.XXXXXX)"
  if [[ -n "$body" ]]; then
...
if status.get("backend") != "microagent":
    raise SystemExit(f"backend = {status.get('backend')!r}, want microagent")
...
for line in lines:
    if line.startswith("model:"):
        out.append("model: microagent-smoke-model")
    else:
        out.append(line)
path.write_text("\n".join(out) + "\n")
PY
  else
    printf '\nmodel: microagent-smoke-model\n' >>"$SMOKE_HOME/agents/$AGENT_NAME/agent.yaml"
  fi
}
...
case "$HOST_OS/$HOST_ARCH" in
  Darwin/arm64) ENFORCER_PLATFORM="darwin/arm64" ;;
  Darwin/x86_64) ENFORCER_PLATFORM="darwin/amd64" ;;
  Linux/aarch64|Linux/arm64) ENFORCER_PLATFORM="linux/arm64" ;;
  Linux/x86_64|Linux/amd64) ENFORCER_PLATFORM="linux/amd64" ;;
  *) fail "unsupported microagent smoke host: $HOST_OS/$HOST_ARCH" ;;
esac

require_cmd curl
require_cmd go
require_cmd "$MICROAGENT_BIN"
require_cmd python3
...
[[ -n "$ROOTFS_OCI_REF" ]] || fail "microagent rootfs OCI artifact is not configured; pass --rootfs-oci-ref or set AGENCY_MICROAGENT_ROOTFS_OCI_REF"
...
if [[ -z "$SMOKE_HOME" ]]; then
  SMOKE_HOME="$(mktemp -d /tmp/agency-microagent-lifecycle.XXXXXX)"
else
  mkdir -p "$SMOKE_HOME"
fi
...
AGENCY_MICROAGENT_BIN="$MICROAGENT_BIN" \
AGENCY_MKE2FS="$MKE2FS_PATH" \
AGENCY_MICROAGENT_ENFORCER_BIN="$ENFORCER_BIN" \
...
STATUS_JSON="$(mktemp /tmp/agency-microagent-status.XXXXXX)"
...
if [[ "$KEEP_AGENT" == "1" ]]; then
  ...
  api_json POST "/api/v1/agents/$AGENT_NAME/stop" '{"type":"immediate","reason":"microagent lifecycle smoke"}' >/dev/null
  api_json DELETE "/api/v1/agents/$AGENT_NAME" >/dev/null
fi
rm -f "$STATUS_JSON"

log "microagent lifecycle smoke passed"
