...
func SaveCapacity(path string, c CapacityConfig) error {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil {
		return fmt.Errorf("create capacity config directory: %w", err)
	}
	content := "# Generated by agency\n" + string(data)
...
// LoadOrProfileCapacity loads capacity.yaml, or profiles and saves a default
// config when the file has not been created yet.
func LoadOrProfileCapacity(path string, hasEmbeddings bool, backend string, backendConfig map[string]string) (CapacityConfig, error) {
	cfg, err := LoadCapacity(path)
	if err == nil {
		return cfg, nil
	}
	...
	cfg, err = ProfileHostForRuntime(hasEmbeddings, backend, backendConfig)
	if err != nil {
		return CapacityConfig{}, err
	}
	if err := SaveCapacity(path, cfg); err != nil {
		return CapacityConfig{}, err
	}
	return cfg, nil
}
