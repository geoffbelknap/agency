...
from agency_core.models.comms import Channel, ChannelState, ChannelType, Message, MessageFlags
...
class MessageStore:
    ...
    def list_channels(self, member: str | None = None, state: str | None = "active") -> list[Channel]:
        channels = []
    ...
    def read_messages(
        ...
        # When no since filter, only parse the tail to avoid wasted Pydantic work
        if not since and len(lines) > limit:
            lines = lines[-limit:]
        messages = []
        for line in lines:
            if not line.strip():
    ...
    def _get_cursor(self, channel: str, participant: str) -> datetime | None:
        cursor_path = self._cursors_dir / f"{participant}.json"
        if not cursor_path.exists():
            return None
        cursors = json.loads(cursor_path.read_text())
