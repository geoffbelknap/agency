...
from agency_core.images.comms.matcher import Matcher
from agency_core.images.comms.store import MessageStore
from agency_core.images.comms.subscriptions import SubscriptionManager
from agency_core.images.comms.websocket import fan_out_message, push_to_agent, setup_websocket
from agency_core.models.comms import ChannelState, ChannelType
from agency_core.models.subscriptions import InterestDeclaration
...
def create_app(data_dir: Path | None = None, agents_dir: Path | None = None) -> web.Application:
    ...
    if mode == "cache":
        ...
        _init_hosts_registry(app)

    return app


def _init_hosts_registry(app: web.Application) -> None:
    """Load known host names for X-Agency-Cache-Relay validation."""
    try:
        from agency_core.swarm.hosts import load_hosts
        hosts = load_hosts()
        app["hosts_registry"] = {h.name for h in hosts}
    except Exception:
        app["hosts_registry"] = None  # No validation if not in swarm mode
...
async def handle_register_expertise(request: web.Request) -> web.Response:
    ...
    from agency_core.models.subscriptions import ExpertiseDeclaration
    try:
