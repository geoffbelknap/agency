...
func (h *handler) spawnMeeseeks(w http.ResponseWriter, r *http.Request) {
	...
	if qp := r.URL.Query().Get("parent"); qp != "" && qp != parent {
		h.audit.Write(parent, "meeseeks_spawn_spoofing_attempt", map[string]interface{}{
	...
	detail, err := h.agents.Show(r.Context(), parent)
	if err != nil {
	...
	mission, err := h.missions.Get(parentMission)
	if err != nil {
	...
	// Spawn via manager
	mks, err := h.meeseeks.Spawn(&req, parent, mission.ID, detail.GrantedCaps, cfg)
	if err != nil {
	...
	h.audit.Write(parent, "meeseeks_spawned", map[string]interface{}{
		"meeseeks_id": mks.ID,
		"task":        mks.Task,
		"model":       mks.Model,
		"budget":      mks.Budget,
...
func (h *handler) killMeeseeks(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" {
		...
		for _, mks := range list {
			_ = h.meeseeks.UpdateStatus(mks.ID, models.MeeseeksStatusTerminated)
			h.meeseeks.Remove(mks.ID)
			killed = append(killed, mks.ID)
		}
		h.audit.Write("_system", "meeseeks_killed_all", map[string]interface{}{
			"parent":  parent,
			"killed":  killed,
			"build_id": h.cfg.BuildID,
		})
	...
	_ = h.meeseeks.UpdateStatus(id, models.MeeseeksStatusTerminated)
...
func (h *handler) completeMeeseeks(w http.ResponseWriter, r *http.Request) {
	...
	_ = h.meeseeks.UpdateStatus(id, models.MeeseeksStatusCompleted)

	h.audit.Write(mks.ParentAgent, "meeseeks_completed", map[string]interface{}{
		"meeseeks_id": id,
		"task":        mks.Task,
		"budget_used": mks.BudgetUsed,
		"build_id":    h.cfg.BuildID,
...
func (h *handler) killMeeseeksByParent(w http.ResponseWriter, r *http.Request) {
	...
	for _, mks := range list {
		...
		h.meeseeks.Remove(mks.ID)
		killed = append(killed, mks.ID)
		h.audit.Write(parent, "meeseeks_terminated", map[string]interface{}{
			"meeseeks_id": mks.ID,
			"task":        mks.Task,
			"budget_used": mks.BudgetUsed,
...
func (h *handler) handleMeeseeksDistress(meeseeksID string, level string, budgetUsed, budgetTotal float64, task, parent string) {
	...
	case "warning":
		...
		h.log.Warn("Meeseeks budget warning",
			"id", meeseeksID,
	...
	case "distress":
		...
		_ = h.meeseeks.UpdateStatus(meeseeksID, models.MeeseeksStatusDistressed)
		h.audit.Write(parent, "meeseeks_distressed", map[string]interface{}{
	...
	case "exhausted":
		...
		h.log.Error("Meeseeks budget exhausted",
			"id", meeseeksID,
			"budget_total", fmt.Sprintf("$%.2f", budgetTotal),
			"task", task,
			"parent", parent,
		)
		_ = h.meeseeks.UpdateStatus(meeseeksID, models.MeeseeksStatusTerminated)
		h.meeseeks.Remove(meeseeksID)
		h.audit.Write(parent, "meeseeks_budget_exhausted", map[string]interface{}{
			"meeseeks_id": meeseeksID,
			"budget":      budgetTotal,
			"task":        task,
		})
