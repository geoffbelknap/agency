...
func (h *handler) connectorConfigure(w http.ResponseWriter, r *http.Request) {
	...
	// Audit
	h.audit.Write(cc.Name, "connector_configure", map[string]interface{}{
...
func (h *handler) loadConnectorConfig(name string) (*models.ConnectorConfig, error) {
	...
	if inst != nil {
		...
		yamlPath = filepath.Join(h.cfg.Home, "connectors", name, "connector.yaml")
	}
...
func (h *handler) isCredentialConfigured(cred models.ConnectorCredential) bool {
	...
	case "env-var":
		...
		return h.envFileHasKey(filepath.Join(h.cfg.Home, ".env"), cred.Name)
	default:
		return false
	}
}
...
func (h *handler) isAuthConfigured(auth *models.ConnectorAuth, requires *models.ConnectorRequires) bool {
	...
	path := filepath.Join(h.cfg.Home, "secrets", "jwt-swap.yaml")
	data, err := os.ReadFile(path)
	if err != nil {
		return false
	}
...
func (h *handler) loadDomainProvenance() map[string]interface{} {
	path := filepath.Join(h.cfg.Home, "egress", "domain-provenance.yaml")
	data, err := os.ReadFile(path)
	if err != nil {
