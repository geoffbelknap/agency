...
func (h *handler) listAgentProcedures(w http.ResponseWriter, r *http.Request) {
	...
	data, err := h.deps.Knowledge.QueryByMission(r.Context(), "entity_type:procedure agent:"+name, "")
	if err != nil {
...
func (h *handler) listAgentEpisodes(w http.ResponseWriter, r *http.Request) {
	name := chi.URLParam(r, "name")
	data, err := h.deps.Knowledge.QueryByMission(r.Context(), "entity_type:episode agent:"+name, "")
	if err != nil {
		writeJSON(w, 502, map[string]string{"error": "knowledge query failed: " + err.Error()})
		return
	}
...
func (h *handler) callEvaluationLLM(taskSummary string, criteria []evaluation.CriterionItem, model string) (evaluation.EvaluationResult, error) {
	...
	addr := h.deps.Config.GatewayAddr
	if addr == "" {
		addr = "127.0.0.1:8200"
	}
	...
	req.Header.Set("X-Agency-Token", h.deps.Config.Token)
...
func (h *handler) evaluateTaskCompletion(agentName string, signalData map[string]interface{}) {
	...
	h.deps.Audit.Write(agentName, "task_evaluation", map[string]interface{}{
	...
	if !result.Passed {
		...
		if h.deps.EventBus != nil {
			h.deps.EventBus.Publish(&models.Event{
				SourceType: "platform",
				SourceName: "gateway",
