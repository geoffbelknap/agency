...
func TestVerifyPactAuditReportChecksSubmittedReport(t *testing.T) {
	home := t.TempDir()
	workspaceDir := filepath.Join(home, "agents", "agent", "workspace")
	resultsDir := filepath.Join(workspaceDir, ".results")
	if err := os.MkdirAll(resultsDir, 0o755); err != nil {
...
`), 0o644); err != nil {
	...
	req := pactRunRequest(http.MethodPost, "/api/v1/agents/agent/pact/runs/task-123/audit-report/verify", "agent", "task-123")
	...
	if invalidBody["valid"] != false || invalidBody["reason"] != "task_id_mismatch" {
		t.Fatalf("unexpected invalid verification: %#v", invalidBody)
	}
}
