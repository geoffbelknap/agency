...
func (h *handler) budgetStore() *budget.Store {
	return budget.NewStore(filepath.Join(h.deps.Config.Home, "budget"))
}
...
func (h *handler) getBudget(w http.ResponseWriter, r *http.Request) {
	...
	// Today's metrics
	todayMetrics, _ := routing.CollectWithCosts(h.deps.Config.Home, routing.MetricsQuery{
		Agent: name,
		Since: todayStart.Format(time.RFC3339),
	}, costs)

	// Monthly metrics
	monthMetrics, _ := routing.CollectWithCosts(h.deps.Config.Home, routing.MetricsQuery{
		Agent: name,
