...
func (h *handler) createOrUpdateCredential(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.deps.CredStore.Put(entry); err != nil {
...
func (h *handler) listCredentials(w http.ResponseWriter, r *http.Request) {
	...
	entries, err := h.deps.CredStore.List(filter)
	if err != nil {
		writeJSON(w, 500, map[string]string{"error": err.Error()})
		return
	}
...
func (h *handler) showCredential(w http.ResponseWriter, r *http.Request) {
	...
	if showValue {
		value = entry.Value
		if h.deps.Audit != nil {
			h.deps.Audit.Write("platform", "credential_value_revealed", map[string]interface{}{
				"credential": name,
				"severity":   "high",
			})
		}
...
func (h *handler) rotateCredential(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.deps.CredStore.Rotate(name, body.Value); err != nil {
		writeJSON(w, 500, map[string]string{"error": err.Error()})
		return
	}
...
func (h *handler) resolveCredential(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range names {
		entry, err := h.deps.CredStore.Get(n)
		if err != nil {
			continue
		}
		resolved, err := credstore.ResolveGroup(*entry, h.deps.CredStore.Backend())
		if err != nil {
		...
		if h.deps.Audit != nil {
			h.deps.Audit.Write("platform", "credential_resolved", map[string]interface{}{
				"credential": n,
				"source":     "store",
				"caller":     "egress",
			})
