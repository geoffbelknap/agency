...
const enabled = process.env.AGENCY_E2E_APPLE_CONTAINER_WEBUI === '1';

test.describe.configure({ timeout: 240_000 });
test.skip(!enabled, 'requires AGENCY_E2E_APPLE_CONTAINER_WEBUI=1 and an Apple Container-backed live stack');
...
type RuntimeStatus = {
  ...
  phase?: string;
  healthy?: boolean;
  transport?: { type?: string; enforcerConnected?: boolean };
...
async function waitForRuntimeHealthy(page: Page, name: string) {
  await expect.poll(async () => {
    ...
    return [
      status.backend,
      status.phase,
      status.transport?.enforcerConnected,
    ].join('|');
  }, { timeout: 120_000, intervals: [2000, 5000, 10000] }).toBe('apple-container|running|true');
}
...
test('operator can create, inspect, DM, stop, and delete an Apple Container agent', async ({ page }) => {
  const agentName = `apple-operator-${Date.now()}`;
  const prompt = `Operator Apple Container pass ${agentName}: reply with one concise sentence.`;
  ...
  try {
    ...
    await expect(page.getByText('apple-container').first()).toBeVisible();

    const manifest = await runtimeManifest(page, agentName);
    const status = await runtimeStatus(page, agentName);
    expect(manifest.spec?.transport?.enforcer?.type).toBe('loopback_http');
    expect(status.backend).toBe('apple-container');
    expect(status.healthy).toBe(true);
    expect(status.transport?.enforcerConnected).toBe(true);
    ...
    await expect(page.getByText(`Delete agent "${agentName}"?`)).toBeVisible();
    await page.getByRole('button', { name: /^Delete agent$/ }).last().click();
    await expect.poll(async () => {
      const response = await page.request.get(`/api/v1/agents/${encodeURIComponent(agentName)}`, {
        headers: await authHeaders(page),
        timeout: 10_000,
