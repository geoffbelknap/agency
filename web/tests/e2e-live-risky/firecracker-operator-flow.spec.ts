...
async function waitForRuntimeHealthy(page: Page, name: string) {
  ...
  }, { timeout: 120_000, intervals: [2000, 5000, 10000] }).toBe('firecracker|running|running|running|running|true');
}
...
test('operator can create, inspect, DM, stop, and delete a Firecracker agent', async ({ page }) => {
  const agentName = `fc-operator-${Date.now()}`;
  const prompt = `Operator Firecracker pass ${agentName}: reply with one concise sentence.`;
  ...
  try {
    ...
    await expect(page.getByText('firecracker').first()).toBeVisible();
    await expect(page.getByText('vsock_http').first()).toBeVisible();
    ...
    expect(manifest.spec?.transport?.enforcer?.endpoint).toMatch(/^vsock:\/\/2:\d+$/);
    expect(status.healthy).toBe(true);
    expect(status.details?.enforcer_component_state).toBe('running');
    expect(status.details?.vsock_bridge_state).toBe('running');
