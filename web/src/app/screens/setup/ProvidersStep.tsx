...
export function ProvidersStep({
  ...
  const handleVerify = async (provider: ProviderData) => {
    const credName = provider.credential_name?.startsWith(`${provider.name}-`)
        ...
        <div className="space-y-4 rounded-2xl border border-border bg-card p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Choose how tier requests should resolve. `Best Effort` is selected by default and works for most first-run setups.
            </p>
          </div>
          <div className="space-y-2">
            {([
