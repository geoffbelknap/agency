...
import { Copy, Package, Pencil, Plus, Save, Trash2, UserPlus } from 'lucide-react';
...
function Card({ children, ariaLabel }: { children: React.ReactNode; ariaLabel?: string }) {
  return (
    <div role={ariaLabel ? 'article' : undefined} aria-label={ariaLabel} style={{ background: 'var(--warm-2)', border: '0.5px solid var(--ink-hairline)', borderLeft: '2px solid transparent', borderRadius: 10, padding: 18, display: 'flex', flexDirection: 'column', gap: 12, minHeight: 210 }}>
      {children}
    </div>
  );
}
...
export function Presets() {
  ...
  const renderPresetCard = (preset: PresetSummary) => {
    ...
    return (
      <Card key={preset.name} ariaLabel={`Preset ${preset.name}`}>
        <div>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 'auto', paddingTop: 8, borderTop: '0.5px solid var(--ink-hairline)' }}>
          ...
          <div style={{ flex: 1 }} />
          {preset.source === 'user' && (
            <DesignButton variant="danger" icon={<Trash2 size={12} />} onClick={() => setDeleteTarget(preset.name)}>Delete</DesignButton>
          )}
