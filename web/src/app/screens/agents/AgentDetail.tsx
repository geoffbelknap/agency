...
import { Play, Pause, RefreshCw, Trash2, X, Loader2 } from 'lucide-react';
...
export function AgentDetail({
  ...
  return (
    <div className="flex flex-col h-full bg-card">
      ...
      {/* Primary tabs */}
      <div role="tablist" className="shrink-0 flex flex-wrap gap-x-1 border-b border-border px-3 py-1.5">
        {([
          ...
          <button key={tab.id} role="tab" aria-selected={primaryTab === tab.id} aria-controls={`panel-${tab.id}`} onClick={() => setPrimaryTab(tab.id)}
            className={`rounded px-3 py-1.5 text-xs font-medium transition-colors whitespace-nowrap ${primaryTab === tab.id ? 'bg-primary/10 text-foreground' : 'text-muted-foreground hover:bg-secondary/60 hover:text-foreground/80'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div role="tabpanel" id={`panel-${primaryTab}`} className="flex-1 overflow-auto">
        {primaryTab === 'overview' && (
          <AgentOverviewTab
            ...
            onRestart={() => onAction(agent.name, 'restart')}
      ...
      <ConfirmDialog
        ...
        title="Delete agent?"
        description={`Delete "${agent.name}" and remove its local runtime resources? Audit history is preserved, but this action cannot be undone from the UI.`}
        confirmLabel="Delete"
        variant="destructive"
