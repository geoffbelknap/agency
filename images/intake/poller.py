...
def hash_blob(data: Union[dict, list]) -> str:
...
def extract_items(data: Union[dict, list], response_key: str) -> Optional[list[dict]]:
...
def apply_transform(items: list, transform_path: Optional[str]) -> list:
    """Apply dot-path extraction to each item.

    Uses same path syntax as extract_items: "$" = identity,
    "$.key" = one level, "$.a.b.c" = nested.
    Returns list of extracted values (empty dict for missing keys).
    """
    if not transform_path or transform_path == "$":
        return items
    path = transform_path.lstrip("$").lstrip(".")
    keys = path.split(".")
    result = []
    for item in items:
        val = item
