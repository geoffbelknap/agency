...
def register_comms_tools(registry, comms_url: str, agent_name: str,
                         artifact_callback=None, artifact_threshold: int = 25) -> None:
    registry.register_tool(
        ...
        handler=lambda args: _send_message(comms_url, agent_name, args,
                                           artifact_callback, artifact_threshold),
    )
...
def _send_message(base_url: str, agent_name: str, args: dict,
                   artifact_callback=None, artifact_threshold: int = 25) -> str:
    content = args["content"]
    metadata = None

    # Apply artifact threshold — long messages get saved as files with a preview
    lines = content.strip().split("\n")
    if artifact_callback and len(lines) > artifact_threshold:
        artifact_id = artifact_callback(content)
        if artifact_id:
