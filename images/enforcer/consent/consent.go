...
func NewPersistentValidator(deploymentID string, keys map[string]ed25519.PublicKey, maxTTL, clockSkew time.Duration, persistPath string) *Validator {
	v := NewValidator(deploymentID, keys, maxTTL, clockSkew)
	v.persistPath = strings.TrimSpace(persistPath)
	_ = v.loadPersistedNonces(time.Now().UTC().UnixMilli())
	return v
}
...
func (v *Validator) consumeNonce(deploymentID, nonce string, expiresAtMs, nowMs int64) bool {
	...
	_ = v.persistLocked(nowMs)
	return false
}

type persistedNonceFile struct {
	Deployments map[string][]persistedNonceEntry `json:"deployments"`
}
...
func (v *Validator) loadPersistedNonces(nowMs int64) error {
	if v.persistPath == "" {
		return nil
	}
	...
	var persisted persistedNonceFile
	if err := json.Unmarshal(data, &persisted); err != nil {
		return err
	}
	for deploymentID, entries := range persisted.Deployments {
		if _, ok := v.consumedNonces[deploymentID]; !ok {
			v.consumedNonces[deploymentID] = make(map[string]int64)
		}
		for _, entry := range entries {
			if nowMs <= entry.ExpiresAt+v.clockSkew.Milliseconds() {
				v.consumedNonces[deploymentID][entry.Nonce] = entry.ExpiresAt
			}
		}
	}
	return nil
...
func (v *Validator) persistLocked(nowMs int64) error {
	if v.persistPath == "" {
		return nil
	}
	persisted := persistedNonceFile{
		Deployments: make(map[string][]persistedNonceEntry, len(v.consumedNonces)),
	}
	for deploymentID, entries := range v.consumedNonces {
		for nonce, expiresAt := range entries {
			if nowMs > expiresAt+v.clockSkew.Milliseconds() {
				delete(v.consumedNonces[deploymentID], nonce)
				continue
			}
			persisted.Deployments[deploymentID] = append(persisted.Deployments[deploymentID], persistedNonceEntry{
				Nonce:     nonce,
				ExpiresAt: expiresAt,
			})
		}
	...
	if err := os.MkdirAll(filepath.Dir(v.persistPath), 0o755); err != nil {
		return err
	}
	tmpPath := v.persistPath + ".tmp"
	if err := os.WriteFile(tmpPath, data, 0o600); err != nil {
		return err
	}
	return os.Rename(tmpPath, v.persistPath)
}
